...
void intercalacao_balanceada_f1() {
    ...
    FILE *fitas[MAX_FITAS];
    Registro buffer[MAX_REGISTROS_2];
    int i;
    ...
    // Simulação de geração de blocos ordenados
    for (i = 0; i < MAX_FITAS - 1; i++) {
        for (int j = 0; j < MAX_REGISTROS_2; j++) {
            buffer[j].inscricao = rand() % 1000000;
            buffer[j].nota = (rand() % 10000) / 100.0;
        }
        qsort(buffer, MAX_REGISTROS_2, sizeof(Registro), comparar_registros);
        fwrite(buffer, sizeof(Registro), MAX_REGISTROS_2, fitas[i]);
    }
